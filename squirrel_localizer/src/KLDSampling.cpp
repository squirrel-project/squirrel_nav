...
#include "squirrel_localizer/ZTable.h"
...
void kld_sampling::build_table() {
  //float tmp;
  //ifstream ifile("/home/roewekae/workspace/omniRob/ztable.data");
  //std::string file_path_and_name;
  // file_path_and_name = ros::package::getPath("ais_ros_localizer") + "/src/kld_sampling/ztable.data";
  // ifstream ifile(file_path_and_name.c_str());
  ...
  //   exit(-1);
  // }
  float table[] = ZTABLE_DEFINITION;
    
  ztable = std::vector<float>(table, table + sizeof(table) / sizeof(float) );
}
