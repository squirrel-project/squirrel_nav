...
class MotionModel {
 public:
  ...
  /// computes the odometry transform from m_lastOdomPose to currentPose as
  /// relative
  ...
  bool lookupLocalTransform(
      const std::string& targetFrame, const ros::Time& t,
      tf::StampedTransform& localTransform) const;

  /// lookup the height of the torso, based on tf between the base and footprint
  /// frames
  ...
  void applyOdomTransform(
      Particles& particles, const tf::Transform& odomTransform);

  /// apply odomTransform to all particles (noisy), with temporal sampling over
  /// the range of dt.
  /// Times are sampled in an interval +-dt/2 around t, iff dt > 0.0.
  bool applyOdomTransformTemporal(
 ...
 protected:
  /// apply odomTransform to one particle pose (with random noise and
  /// calibration)
  void applyOdomTransform(
      tf::Pose& particlePose, const tf::Transform& odomTransform);

  /// Transform a particle's pose with the relative odomTransform with added
  /// random noise
  void transformPose(
      tf::Pose& particlePose, const tf::Transform& odomTransform);
