...
boost::shared_ptr<octomap::OcTree> MapModel::getMap() const { return m_map; }
...
#pragma omp parallel for
  for (unsigned i = 0; i < particles.size(); ++i) {
    ...
    octomap::point3d position(
        particles[i].pose.getOrigin().getX(),
        particles[i].pose.getOrigin().getY(),
        particles[i].pose.getOrigin().getZ());

    // see if outside of map bounds:
    if (position(0) < minX || position(0) > maxX || position(1) < minY ||
        position(1) > maxY || position(2) < minZ || position(2) > maxZ) {
...
#pragma omp atomic
        ...
        if (m_motionRangeZ >= 0.0 && (std::abs(
                                          particles[i].pose.getOrigin().getZ() -
                                          getFloorHeight(particles[i].pose) -
                                          m_motionMeanZ) > m_motionRangeZ)) {
          particles[i].weight = minWeight;
#pragma omp atomic
          numMotion++;
          ...
          if ((m_motionRangePitch >= 0.0 &&
               std::abs(pitch) > m_motionRangePitch) ||
              (m_motionRangeRoll >= 0.0 &&
               std::abs(roll) > m_motionRangeRoll)) {
            particles[i].weight = minWeight;
#pragma omp atomic
            numMotion++;
          }
  ...
  if (numWall > 0 || numOut > 0 || numMotion > 0) {
    ROS_INFO(
        "Particle weights minimized: %d out of map, %d in obstacles, %d out of "
        "motion range",
        numOut, numWall, numMotion);
  }
...
void MapModel::initGlobal(
    Particles& particles, double z, double roll, double pitch,
    const Vector6d& initNoise, UniformGeneratorT& rngUniform,
  ...
  while (true) {
    ...
    for (unsigned zIdx = 0; zIdx < z_list.size(); zIdx++) {
      ...
      it->pose.getOrigin().setZ(
          z_list.at(zIdx) + z + rngNormal() * initNoise(2));
...
DistanceMap::DistanceMap(ros::NodeHandle* nh) : MapModel(nh) {
  ...
  //  octomap_msgs::fullMsgDataToMap(resp.map.data);
  //  if (tree){
  ...
  octomap::OcTree* tree = dynamic_cast<octomap::OcTree*>(
      octomap::AbstractOcTree::read(mapFileName));
  if (tree) {
    m_map.reset(tree);
  }
...
double OccupancyMap::getFloorHeight(const tf::Transform& pose) const {
  octomap::point3d end;
  if (m_map->castRay(
          octomap::pointTfToOctomap(pose.getOrigin()),
          octomap::point3d(0.0, 0.0, -1.0), end, false)) {
