...
SquirrelLocalizer::SquirrelLocalizer(unsigned randomSeed)
    : m_rngEngine(randomSeed),
      ...
      m_nEffFactor(1.0),
      m_minParticleWeight(0.0),
      m_bestParticleIdx(-1),
      m_lastIMUMsgBuffer(5),
      ...
      m_observationThresholdTrans(0.1),
      m_observationThresholdRot(M_PI / 6),
      m_observationThresholdHeadYawRot(0.5),
      m_observationThresholdHeadPitchRot(0.3),
      m_temporalSamplingRange(0.1),
      m_transformTolerance(0.1),
      m_groundFilterPointCloud(true),
      m_groundFilterDistance(0.04),
      m_groundFilterAngle(0.15),
      m_groundFilterPlaneDistance(0.07),
      m_sensorSampleDistGroundFactor(3),
      m_headYawRotationLastScan(0.0),
      m_headPitchRotationLastScan(0.0),
      m_useIMU(false),
      m_constrainMotionZ(false),
      m_constrainMotionRP(false),
      m_useTimer(false),
      m_timerPeriod(0.1) {
  ...
  m_privateNh.param(
      "min_particle_weight", m_minParticleWeight, m_minParticleWeight);

  m_privateNh.param("initial_pose/x", m_initPose(0), 0.0);
  m_privateNh.param("initial_pose/y", m_initPose(1), 0.0);
  m_privateNh.param(
      "initial_pose/z", m_initPose(2), 0.32);  // hip height when standing
  ...
  m_privateNh.param(
      "sensor_sampling_dist", m_sensorSampleDist, m_sensorSampleDist);
  m_privateNh.param("max_range", m_filterMaxRange, 30.0);
  m_privateNh.param("min_range", m_filterMinRange, 0.05);
  ROS_DEBUG(
      "Using a range filter of %f to %f", m_filterMinRange, m_filterMaxRange);

  m_privateNh.param(
      "update_min_trans", m_observationThresholdTrans,
      m_observationThresholdTrans);
  m_privateNh.param(
      "update_min_rot", m_observationThresholdRot, m_observationThresholdRot);
  m_privateNh.param(
      "update_min_head_yaw", m_observationThresholdHeadYawRot,
      m_observationThresholdHeadYawRot);
  m_privateNh.param(
      "update_min_head_pitch", m_observationThresholdHeadPitchRot,
      m_observationThresholdHeadPitchRot);
  m_privateNh.param(
      "temporal_sampling_range", m_temporalSamplingRange,
      m_temporalSamplingRange);
  m_privateNh.param(
      "transform_tolerance", m_transformTolerance, m_transformTolerance);

  m_privateNh.param("use_imu", m_useIMU, m_useIMU);
  m_privateNh.param(
      "constrain_motion_z", m_constrainMotionZ, m_constrainMotionZ);
  m_privateNh.param(
      "constrain_motion_rp", m_constrainMotionRP, m_constrainMotionRP);

  // point cloud observation model parameters
  m_privateNh.param(
      "ground_filter_point_cloud", m_groundFilterPointCloud,
      m_groundFilterPointCloud);
  m_privateNh.param(
      "ground_filter_distance", m_groundFilterDistance, m_groundFilterDistance);
  m_privateNh.param(
      "ground_filter_angle", m_groundFilterAngle, m_groundFilterAngle);
  m_privateNh.param(
      "ground_filter_plane_distance", m_groundFilterPlaneDistance,
      m_groundFilterPlaneDistance);
  m_privateNh.param(
      "sensor_sampling_dist_ground_factor", m_sensorSampleDistGroundFactor,
  ...
  m_posePub =
      m_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("pose", 10);
  m_poseEvalPub =
      m_nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("pose_eval", 10);
  m_poseOdomPub =
      m_privateNh.advertise<geometry_msgs::PoseStamped>("pose_odom_sync", 10);
  m_poseArrayPub =
      m_privateNh.advertise<geometry_msgs::PoseArray>("particlecloud", 10);
  m_bestPosePub =
      m_privateNh.advertise<geometry_msgs::PoseArray>("best_particle", 10);
  m_nEffPub = m_privateNh.advertise<std_msgs::Float32>("n_eff", 10);
  m_filteredPointCloudPub =
      m_privateNh.advertise<sensor_msgs::PointCloud2>("filtered_cloud", 1);

  // TODO Propagate particles independent of sensor callback
  reset();

  // ROS subscriptions last:
  m_globalLocSrv = m_nh.advertiseService(
  ...
  m_laserSub = new message_filters::Subscriber<sensor_msgs::LaserScan>(
      m_nh, "scan", 100);
  m_laserFilter = new tf::MessageFilter<sensor_msgs::LaserScan>(
      *m_laserSub, m_tfListener, m_odomFrameId, 100);
  m_laserFilter->registerCallback(
      boost::bind(&SquirrelLocalizer::laserCallback, this, _1));

  // subscription on point cloud, tf message filter
  m_pointCloudSub = new message_filters::Subscriber<sensor_msgs::PointCloud2>(
      m_nh, "point_cloud", 100);
  m_pointCloudFilter = new tf::MessageFilter<sensor_msgs::PointCloud2>(
  ...
  m_initPoseSub =
      new message_filters::Subscriber<geometry_msgs::PoseWithCovarianceStamped>(
          m_nh, "initialpose", 2);
  m_initPoseFilter =
      new tf::MessageFilter<geometry_msgs::PoseWithCovarianceStamped>(
          *m_initPoseSub, m_tfListener, m_globalFrameId, 2);
  m_initPoseFilter->registerCallback(
      boost::bind(&SquirrelLocalizer::initPoseCallback, this, _1));

  m_pauseIntegrationSub = m_privateNh.subscribe(
      "pause_localization", 1, &SquirrelLocalizer::pauseLocalizationCallback,
      this);
  m_pauseLocSrv = m_privateNh.advertiseService(
      "pause_localization_srv",
      &SquirrelLocalizer::pauseLocalizationSrvCallback, this);
  m_resumeLocSrv = m_privateNh.advertiseService(
      "resume_localization_srv",
      &SquirrelLocalizer::resumeLocalizationSrvCallback, this);

  if (m_useIMU)
    m_imuSub = m_nh.subscribe("imu", 5, &SquirrelLocalizer::imuCallback, this);
  if (m_useTimer) {
...
void SquirrelLocalizer::timerCallback(const ros::TimerEvent& e) {
  ros::Time transformExpiration =
      e.current_real + ros::Duration(m_transformTolerance);
  tf::StampedTransform tmp_tf_stamped(
      m_latest_transform, transformExpiration, m_globalFrameId,
      m_targetFrameId);
  m_tfBroadcaster.sendTransform(tmp_tf_stamped);
}

void SquirrelLocalizer::reset() {
  ...
  if (m_initGlobal) {
    ...
    if (m_initFromTruepose) {  // useful for evaluation, when ground truth
                               // available:
      ...
      tf::Stamped<tf::Pose> ident(
          ...
          ros::Time::now(), "torso_real");  // TODO: param
      ...
      for (int j = 0; j < 6; ++j) {
        for (int i = 0; i < 6; ++i) {
          if (i == j)
            posePtr->pose.covariance.at(i * 6 + j) =
...
void SquirrelLocalizer::initZRP(double& z, double& roll, double& pitch) {
...
void SquirrelLocalizer::laserCallback(
  ...
  if (!m_paused &&
    ...
    sensor_integrated =
        localizeWithMeasurement(pc_filtered, laserRangesSparse, msg->range_max);
  }

  if (!sensor_integrated) {  // no laser integration: propagate particles
...
void SquirrelLocalizer::constrainMotion(const tf::Pose& odomPose) {
...
#pragma omp parallel for
  for (unsigned i = 0; i < m_particles.size(); ++i) {
    ...
    if (m_constrainMotionRP) {
      ...
      m_particles[i].pose.setRotation(
          tf::createQuaternionFromRPY(odomRoll, odomPitch, yaw));
    }
  }
}

bool SquirrelLocalizer::isAboveMotionThreshold(const tf::Pose& odomPose) {
  ...
  return (
      odomTransform.getOrigin().length() >= m_observationThresholdTrans ||
      std::abs(yaw) >= m_observationThresholdRot);
}

bool SquirrelLocalizer::localizeWithMeasurement(
  ...
  m_motionModel->applyOdomTransformTemporal(
      m_particles, t, m_temporalSamplingRange);
  ...
  // this takes the latest tf, assumes that torso to sensor did not change over
  // temp. sampling!
  tf::StampedTransform localSensorFrame;
  if (!m_motionModel->lookupLocalTransform(
          pc_filtered.header.frame_id, t, localSensorFrame))
    return false;
  ...
  m_observationModel->integrateMeasurement(
      m_particles, pc_filtered, ranges, max_range, torsoToSensor);
...
// build a point cloud
  ...
  for (unsigned beam_idx = 0; beam_idx < numBeams; beam_idx += step) {
    ...
    if (range >= laserMin && range <= m_filterMaxRange) {
      ...
      tf::Transform laserAngleRotation(
          tf::Quaternion(tf::Vector3(0.0, 0.0, 1.0), laserAngle));
...
void SquirrelLocalizer::prepareLaserPointCloud(
  ...
  beams_projector_.transformLaserScanToPointCloud(
      targetFrame, *laser, pc_scan, m_tfListener);
...
void SquirrelLocalizer::filterGroundPlane(
  ...
  if (pc.size() < 50) {
    ...
    // TODO: maybe a filtering based on the surface normals might be more robust
    // / accurate?
    ...
    while (cloud_filtered.size() > 10 && !groundPlaneFound) {
      ...
      if (std::abs(coefficients->values.at(3)) < groundFilterPlaneDistance) {
        ...
        ROS_DEBUG(
            "Horizontal plane (not ground) found: %zu/%zu inliers. Coeff: %f "
            "%f %f %f",
            inliers->indices.size(), cloud_filtered.size(),
            coefficients->values.at(0), coefficients->values.at(1),
            coefficients->values.at(2), coefficients->values.at(3));
        ...
        //            writer.write<pcl::PointXYZ>("nonground_plane.pcd",cloud_out,
        //            false);
    ...
    if (!groundPlaneFound) {  // no plane found or remaining points too small
      ...
      second_pass.setFilterLimits(
          -groundFilterPlaneDistance, groundFilterPlaneDistance);
    ...
    //          writer.write<pcl::PointXYZ>("nonground.pcd",pc_nonground,
    //          false);
  }
}

void SquirrelLocalizer::prepareGeneralPointCloud(
  ...
  try {
    m_tfListener.waitForTransform(
        m_baseFootprintId, msg->header.frame_id, msg->header.stamp,
        ros::Duration(0.2));
    m_tfListener.lookupTransform(
        m_baseFootprintId, msg->header.frame_id, msg->header.stamp,
        sensorToBaseFootprint);
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr pcd_tmp(
  ...
  if (m_groundFilterPointCloud) {
    ...
    if (ground.size() > 0) {  // check for 0 size, otherwise PCL crashes
      ...
      voxelGridSampling(
          ground, sampledIndices,
          m_sensorSampleDist * m_sensorSampleDistGroundFactor);
    ...
    if (m_verbose)
      ROS_INFO_STREAM(
          m_nodeName << "PointCloudGroundFiltering done. Added "
                     ...
                     << numFloorPoints << "ground points (from "
                     << ground.size() << "). Cloud size is " << pc.size());
...
void SquirrelLocalizer::pointCloudCallback(
  ...
  if (!m_motionModel->lookupLocalTransform(
          msg->header.frame_id, msg->header.stamp, torsoToSensor))
  ...
  if (!m_paused && (!m_receivedSensorData || isAboveHeadMotionThreshold ||
    ...
    double maxRange = 10.0;  // TODO #4: What is a maxRange for pointClouds?
                             // NaN? maxRange is expected to be a double and
                             // integrateMeasurement checks rangesSparse[i] >
                             // maxRange
    ROS_DEBUG(
        "Updating Pose Estimate from a PointCloud with %zu points and %zu "
        "ranges",
        pc_filtered.size(), rangesSparse.size());
    sensor_integrated =
        localizeWithMeasurement(pc_filtered, rangesSparse, maxRange);
  }
...
void SquirrelLocalizer::synchronizedCallback(
  ...
  ROS_INFO_STREAM_COND(
      m_printSynchronizedSubscription,
...
bool SquirrelLocalizer::toggleSensorsSrvCallback(
  ...
  if (useLaserScanner && useDepthCamera &&
      (!m_useLaserScanner && !m_useDepthCamera))
    m_printSynchronizedSubscription = true;
  if (!useLaserScanner && !useDepthCamera)
    ROS_WARN_STREAM(m_nodeName << ": Subscription to both laser scanner and "
                                  "depth camera have been disable. Localizing "
                                  "with dead reckoning.");
...
bool SquirrelLocalizer::resetMapSrvCallback(
  ...
  tf::poseMsgToTF(req.tf_oldMap2newMap, tf_newMap2oldMap);
  m_poseArray.header.frame_id = m_globalFrameId;
...
bool SquirrelLocalizer::getImuMsg(
  ...
  ItT closestOlder         = m_lastIMUMsgBuffer.end(),
      closestNewer         = m_lastIMUMsgBuffer.end();
  for (ItT it = m_lastIMUMsgBuffer.begin(); it != m_lastIMUMsgBuffer.end();
       it++) {
  ...
  if (closestOlderStamp < maxAge && closestNewerStamp < maxAge &&
      closestOlderStamp + closestNewerStamp > 0.0) {
    // Linear interpolation
    const double weightOlder =
        closestNewerStamp / (closestNewerStamp + closestOlderStamp);
    const double weightNewer = 1.0 - weightOlder;
    imuStamp                 = ros::Time(
        weightOlder * closestOlder->header.stamp.toSec() +
    ...
    ROS_DEBUG(
        ...
        closestOlder->header.stamp.toSec(), imuStamp.toSec(),
        closestNewer->header.stamp.toSec());
    return true;
  } else if (closestOlderStamp < maxAge || closestNewerStamp < maxAge) {
    // Return closer one
    ItT it =
        (closestOlderStamp < closestNewerStamp) ? closestOlder : closestNewer;
...
void SquirrelLocalizer::initPoseCallback(
  ...
  if (std::abs(pose.getOrigin().getZ()) < 0.01) {
    ...
    if (m_initPoseRealZRP) {
      ...
      if (stamp.isZero()) {
        // Header stamp is not set (e.g. RViz), use stamp from latest pose
        // message instead
        tf::Stamped<tf::Pose> lastOdomPose;
      ...
      if (m_useIMU) {
        if (m_lastIMUMsgBuffer.empty()) {
          ...
          if (msg->header.stamp.isZero()) {
            // Header stamp is not set (e.g. RViz), use stamp from latest IMU
            // message instead
            getRP(m_lastIMUMsgBuffer.back().orientation, roll, pitch);
            ok = true;
          } else {
  ...
  for (Particles::iterator it = m_particles.begin(); it != m_particles.end();
    ...
    Vector6d poseCovNoise =
        initCovL * poseNoise;  // is now drawn according to covariance noise
    ...
    transformNoise.setOrigin(
        tf::Vector3(poseCovNoise(0), poseCovNoise(1), poseCovNoise(2)));
    tf::Quaternion q;
    ...
    if (heights.size() > 0) {
      ...
      it->pose.getOrigin().setZ(
          heights.at(int(double(idx) / m_particles.size() * heights.size())) +
          poseHeight);
    }
...
bool SquirrelLocalizer::globalLocalizationCallback(
...
void SquirrelLocalizer::normalizeWeights() {
...
double SquirrelLocalizer::getCumParticleWeight() const {
  double cumWeight = 0.0;

  // compute the cumulative weights
  for (Particles::const_iterator it = m_particles.begin();
...
void SquirrelLocalizer::resample(unsigned numParticles) {
...
void SquirrelLocalizer::publishPoseEstimate(
...
#pragma omp parallel for
  ...
  tf::Transform latestTF(
      ...
      tf::Point(targetToMapTF.getOrigin()));
  ...
  tf::StampedTransform tmp_tf_stamped(
      latestTF.inverse(), transformExpiration, m_globalFrameId,
      m_targetFrameId);
...
tf::Pose SquirrelLocalizer::getParticlePose(unsigned particleIdx) const {
  return m_particles.at(particleIdx).pose;
}

tf::Pose SquirrelLocalizer::getBestParticlePose() const {
  return getParticlePose(getBestParticleIdx());
}

tf::Pose SquirrelLocalizer::getMeanParticlePose() const {
  ...
  meanPose.getBasis() = meanPose.getBasis().scaled(tf::Vector3(
      1.0 / m_numParticles, 1.0 / m_numParticles, 1.0 / m_numParticles));
...
double SquirrelLocalizer::nEff() const {
...
void SquirrelLocalizer::toLogForm() {
...
void SquirrelLocalizer::pauseLocalizationCallback(
...
bool SquirrelLocalizer::pauseLocalizationSrvCallback(
    std_srvs::Empty::Request& req, std_srvs::Empty::Response& res) {
  if (!m_paused) {
    m_paused = true;
    ROS_INFO_STREAM(m_nodeName << ": Localization paused");
...
bool SquirrelLocalizer::resumeLocalizationSrvCallback(
    std_srvs::Empty::Request& req, std_srvs::Empty::Response& res) {
  if (m_paused) {
    m_paused = false;
    ROS_INFO_STREAM(m_nodeName << ": Localization resumed");
...
bool SquirrelLocalizer::lookupPoseHeight(
